...
(function() {
    ...
    Gauge = (function(superClass) {
      ...
      Gauge.prototype.renderStaticLabels = function(staticLabels, w, h, radius) {
        var font, fontsize, j, len, match, re, ref, rest, rotationAngle, value, lineHeight;
        this.ctx.save();
        ...
        for (j = 0, len = ref.length; j < len; j++) {
          ...
          if (value.label !== void 0) {
            // console.log(value, Math.cos(this.getAngle(value.value))*radius)
            if ((!this.options.limitMin || value.value >= this.minValue) && (!this.options.limitMax || value.value <= this.maxValue)) {
              ...
              // this.ctx.textAlign = 'center';
              // this.ctx.translate(Math.cos(this.getAngle(value.value))*radius + lineHeight, -Math.sin(this.getAngle(value.value))*radius - lineHeight);
